...
{% macro validate_dq_env_var(env_var_name) %}
{% set value=env_var(env_var_name) %}
{% if value is none or value|trim=='' %}{{ exceptions.raise_compiler_error("Required env var '"~env_var_name~"' is empty for the DQ framework.") }}{% endif %}
{% if value==env_var_name %}{{ exceptions.raise_compiler_error("Env var '"~env_var_name~"' is set to the placeholder token instead of a real database name.") }}{% endif %}
{{ return(value) }}
{% endmacro %}

{% macro resolve_dq_relation_name(relation_name) %}
{% set ns=namespace(resolved=relation_name) %}
{% set replacements={
'DBT_DPPL_DATABASE_NAME':validate_dq_env_var('DBT_DPPL_DATABASE_NAME'),
'DBT_SSCTA_DATABASE_NAME':validate_dq_env_var('DBT_SSCTA_DATABASE_NAME'),
'DBT_SSCTA_SSC_GDP_SHARE_DATABASE_NAME':validate_dq_env_var('DBT_SSCTA_SSC_GDP_SHARE_DATABASE_NAME')
} %}
{% for token,value in replacements.items() %}
{% set ns.resolved=ns.resolved|replace(token,value) %}
{% set ns.resolved=ns.resolved|replace('{{'~token~'}}',value) %}
{% endfor %}
{% do log("DQ relation before resolve: "~relation_name,info=True) %}
{% do log("DQ relation after resolve: "~ns.resolved,info=True) %}
{{ return(ns.resolved) }}
{% endmacro %}
...
{% set src=resolve_dq_relation_name(src) %}{% set tgt=resolve_dq_relation_name(tgt) %}{% set table_name=resolve_dq_relation_name(table_name) %}
...
{% set ex_s=[] %}{% set ex_t=[] %}
{% for row in cfg.rows %}
{% if resolve_dq_relation_name(row[0])==table_name %}
...
{% set src=resolve_dq_relation_name(src) %}{% set tgt=resolve_dq_relation_name(tgt) %}
...
{% set src=resolve_dq_relation_name(src) %}{% set tgt=resolve_dq_relation_name(tgt) %}{% set k=keys|join(',') %}
...
{% set src=resolve_dq_relation_name(src) %}{% set tgt=resolve_dq_relation_name(tgt) %}{% set cond=[] %}
...
{% for row in r.rows %}
{% set src_table=resolve_dq_relation_name(row[0]) %}{% set tgt_table=resolve_dq_relation_name(row[1]) %}{% set sql=resolve_dq_relation_name(row[3]) %}{% set cnt=get_count(sql|replace(';','')) %}
...
{% for r in cfg.rows %}
{% set src=resolve_dq_relation_name(r[1]) %}
{% set tgt=resolve_dq_relation_name(r[2]) %}
